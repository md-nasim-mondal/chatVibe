...
const CheckoutForm: React.FC<CheckoutFormProps> = ({ price }) => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const card = elements.getElement(CardElement);
    if (!card) {
      setError("Payment card element not found.");
      return;
    }
    ...
    try {
      ...
      // Save payment details (update this as needed based on your backend)
      savePayment(user, price, "USD");

      // Construct the full API URL
      const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/payment/premium?email=${emailAddresses}`;
      console.log("Requesting URL:", apiUrl); // Debugging URL

      // Make the Axios request to update premium status
      await axios.put(apiUrl, { isPremium: true });
